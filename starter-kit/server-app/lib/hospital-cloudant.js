...
/**
 * Find all hospitals that match the specified partial name.
 *
 * @param {String} type
 ...
 */
function find(type, hospitalName, hospitalID, district, state, country) {
  return new Promise((resolve, reject) => {
    let selector = {};
    if (type) {
      selector["type"] = type;
    ...
    if (hospitalName) {
      let search = `(?i).*${hospitalName}.*`;
      selector["name"] = { $regex: search };
    }
    if (hospitalID) {
      selector["hospitalID"] = hospitalID;
    }
    if (district) {
...
/**
 * Delete a hospital that matches a ID.
 *
 * @param {String} id
 *
 * @return {Promise} Promise -
    ...
    db.get(id, (err, document) => {
      if (err) {
        ...
        if (availability) {
          ...
          item["availability"] = document.availability;
        }
        if (district) {
          item["district"] = district;
        } else {
          item["district"] = document.district;
        }
        if (contact) {

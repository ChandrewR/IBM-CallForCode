 ...
 */
function find(type, district, state, country, userID) {
    return new Promise((resolve, reject) => {
        let selector = {}
        if (type) {
            selector['type'] = type;
 ...
 */
function create(type, district, state, country, location, contact, userID, currentcoronacount) {
    return new Promise((resolve, reject) => {
        let itemId = uuidv4();
        ...
        let item = {
            ...
            state: state,
            country: country,
            currentcoronacount: currentcoronacount,
            location: location,
            contact: contact,
            userID: userID,
 ...
 * when the call to the DB completes
 */
function update(id, type, district, state, country, location, contact, userID, currentcoronacount) {
    return new Promise((resolve, reject) => {
        db.get(id, (err, document) => {
            if (err) {
                ...
                if (currentcoronacount) { item["currentcoronacount"] = currentcoronacount } else { item["currentcoronacount"] = document.currentcoronacount };

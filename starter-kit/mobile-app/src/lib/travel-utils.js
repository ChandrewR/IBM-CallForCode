...
export const search = (query) => {
  ...
  const district = query.district ? `name=${query.district}` : ''
  const state = query.state ? `name=${query.state}` : ''
  const country = query.country ? `name=${query.country}` : ''
  const userID = query.userID ? `userID=${query.userID}` : ''

  return fetch(`${serverUrl}/api/travel?${district}&${type}&${userID}&${state}&${country}`, {
    method: 'GET',
    mode: 'no-cors',
    cache: 'no-cache',
    headers: {
...
export const add = (item) => {
  return fetch(`${serverUrl}/api/travel`, {
    method: 'POST',
    mode: 'no-cors',
    cache: 'no-cache',
    headers: {
...
export const update = (item) => {
  return fetch(`${serverUrl}/api/travel/${item.id}`, {
    method: 'PATCH',
    mode: 'no-cors',
    cache: 'no-cache',
    headers: {
